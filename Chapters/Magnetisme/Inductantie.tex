...
\begin{app}[RLC-kringen]{RLC-kringen}
    ...
    \vspace{-0.25cm}
    \begin{equation*}
        - LI\frac{dI}{dt} - IR + \frac{Q}{C} = 0 \ \Rightarrow \ L\frac{d^2Q}{dt^2} + R\dfrac{dQ}{dt} + \dfrac{Q}{C} = 0
    \end{equation*}
...
\begin{theo}[Wisselstroomkringen]{Wisselstroomkringen}
    Wisselstroomkringen (AC-kringen) zijn kringen waarbij de stroom varieert in de tijd. De stroom en het potentiaalverschil van de bron zijn sinusvormige functies
    \begin{equation*}
        i = I_{\max}\sin(\omega t + \phi) \ \Rightarrow \ \Delta v = \Delta V_{\max}\sin(\omega t + \phi)
    \end{equation*}
    met amplitude $I_\text{max}$ voor de stroom, $\Delta V_{\max}$ voor het potentiaalverschil. De rms-waarden van een sinusvormige wisselstroom zijn gedefinieerd als
    \begin{equation*}
        I_{\text{rms}} = \dfrac{I_{\max}}{\sqrt{2}} \ \Rightarrow \ \Delta V_{\text{rms}} = \dfrac{\Delta V_{\max}}{\sqrt{2}}
    \end{equation*}
    wat neerkomt op de DC-output van de AC-bron.
    
    % \noindent \textbf{Opmerking:} de rms-waarden van een sinusvormige wisselstroom zijn gedefinieerd als
    % \begin{equation*}
    %     I_{\text{rms}} = \dfrac{I_{\max}}{\sqrt{2}} \ \Rightarrow \ \Delta V_{\text{rms}} = \dfrac{\Delta V_{\max}}{\sqrt{2}}
    % \end{equation*}
    % wat neerkomt op de DC-output van de AC-bron.
    % De stroom kan worden beschreven door
    % \begin{equation*}
    %     i = I_{\max}\sin(\omega t)
    % \end{equation*}
    % met $I_\text{max}$, het potentiaalverschil van de bron kan beschreven worden door
    % \begin{equation*}
...
\begin{pro}[Vermogen in een wisseltroomkring]{Vermogen in een wisseltroomkring}
    ...
    \begin{equation*}
        P_{\text{gem}} = \frac{1}{2}I_{\max}\Delta V_{\max}\cos(\phi) = I_{\text{rms}} \Delta V_{\text{rms}} \cos(\phi).
    \end{equation*}
    % \noindent \textbf{Opmerking:} de rms-waarde van een sinusvormige wisselstroom is gedefinieerd als: 
    % \begin{equation*}
    %     I_{\text{rms}} = \dfrac{I_{\max}}{\sqrt{2}} \quad \text{en} \quad \Delta V_{\text{rms}} = \dfrac{\Delta V_{\max}}{\sqrt{2}}
    % \end{equation*}
    % \vspace{-0.3cm}
\end{pro}
